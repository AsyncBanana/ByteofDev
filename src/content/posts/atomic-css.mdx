...
[Tailwind](https://tailwindcss.com/) is an Atomic CSS framework created by Adam Wathan in 2017. Unlike Tachyons, it ran on PostCSS, a plugin-based CSS preprocessor. That allowed Tailwind to integrate with other PostCSS plugins and even made it so you could use Tailwind classes inside of CSS using `@apply . ## ### #### \{ to grid \{class\} Additionally, Tailwind was much more configurable and extendable. Because of these advantages, grew far past both Tachyons ACSS is currently at million weekly downloads. Now, let s look why people

use Atomic CSS. Why People CSS

Encapsulation With

CSS, your automatically encapsulated. styling in classes, it only applies specifically the elements that have classes. This encapsulation means you never worry about accidentally breaking something because removed a rule button class certain required. can know which buttons need what they their own classes for particular needs. one main reasons created still helpful this day. Concise Developers are always looking way write least code. Whether Sass, Stylus, or LESS, preprocessors tried decrease code needed. However, even if remove brackets semicolons. significantly smaller rules tool generate frameworks like condense media queries

into simple two-letter

prefix. For example, how would build responsive layout Tailwind. css div display: ~2 ;
