...
By default, text will not appear in many browsers until the correct font is loading. That behavior is called Flash of Invisible Text, or FOIT. Some other browsers display text using the fallback font while it is loading, ensuring text is not invisible but can cause a [layout shift](https://web.dev/optimize-cls/#web-fonts-causing-foutfoit).
