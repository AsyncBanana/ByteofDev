---
title: "Why I hate JavaScript numbers and what everyone has (not) done with them"
...
Imagine you are implementing an ID system like [Twitter's Snowflake](https://blog.x.com/engineering/en_us/a/2010/announcing-snowflake). Everything fits perfectly into a 64-bit integer—41 bits used for a millisecond-precision unix timestamp, 10 bits for a server ID, and the remaining 12 bits used to increment for each new ID within a given server-millisecond. You implement these IDs in your JavaScript app, but wait... your IDs keep rounding off. The problem? When handling numbers, JavaScript acts like a _special snowflake_ (sorry).

As you can probably tell, I encountered this issue pretty recently, and it reminded me why I so strongly dislike JavaScript's numbers, or, more accurately, the JavaScript ecosystem's ignorance when it comes to solving JavaScript's number problems.

> **If you already know about JavaScript's number problems, you can skip to why (Spoiler alert!) [BigInts haven't quite solved the problem (yet)](#what-about-bigint?)**
...
Think back to the scenario I presented earlier of implementing a 64-bit snowflake ID. The number never overflows, instead beginning to round itself because it exceeds the floating-point's 53 bits of precision. The rounding due to the exponent adjsuting itself to store the large number, increasing above ±2^53 and causing the smallest possible increment goes from 1 to 2, to 4, and so on.
...
The use of floating-point numbers also causes a lot of other strange JavaScript behaviors. `NaN`, everyone's favorite bug-creator, exists as part of the IEEE floating-point standard used by JavaScript. This part of the standard was designed to handle arithmetic errors (`0/0`) more efficientlyl, but JavaScript takes that and massively expands upon its use, despite JavaScript not being fast enough in the first place to justify using `NaN` instead of throwing an exception.
...
I can already hear Redditor angrily typing _BigInts solve all of these problems!_. While that is true, they are limited by the language and, even more significantly, by the JavaScript ecosystem.
